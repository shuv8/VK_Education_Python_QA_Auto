    ...
    def click_btn(self, locator, timeout=None):
        ...
        for i in range(self.CLICK_RETRY):
            try:
                ...
                self.logger.warning(f'StaleElementReferenceException caught when clicking, number of try={i+1}')
                if i == self.CLICK_RETRY - 1:
                    raise
            except ElementClickInterceptedException:
                self.logger.warning(f'ElementClickInterceptedException caught when clicking, number of try={i+1}')
                if i == self.CLICK_RETRY - 1:
                    raise
